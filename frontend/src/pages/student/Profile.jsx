...
import { User, Mail, Phone, Building2, Save, GraduationCap } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in max-w-3xl">
      ...
      <Card>
        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100">
          <div className="w-20 h-20 bg-gradient-to-br from-violet-600 to-violet-500 rounded-2xl flex items-center justify-center text-white text-3xl font-bold shadow-lg">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${user?.status === 'approved' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                {user?.status}
              </span>
              {user?.student_id && <span className="text-xs text-slate-500">ID: {user.student_id}</span>}
              {user?.semester && <span className="text-xs text-slate-500">Sem {user.semester}</span>}
            </div>
          </div>
        </div>
